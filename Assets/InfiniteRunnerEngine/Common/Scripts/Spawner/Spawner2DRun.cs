...
{	
	...
	public class Spawner2DRun : MonoBehaviour 
	{
		...
		public virtual GameObject Spawn(Vector3 spawnPosition,bool triggerObjectActivation=true)
		{
            ...
            nextGameObject.gameObject.GetComponent<SpriteRenderer>().sprite = sprite[UnityEngine.Random.Range(0, sprite.Length)];
            /// we rescale the object
            Vector3 newScale;
            float ratio = nextGameObject.transform.localScale.x / nextGameObject.transform.localScale.y;

            if (!PreserveRatio)
	        {
                float xx = UnityEngine.Random.Range(MinimumSize.x, MaximumSize.x);
                float yy = xx * ratio;
                newScale = new Vector3 (xx, yy, UnityEngine.Random.Range (MinimumSize.z, MaximumSize.z));
			}
			else
			{
