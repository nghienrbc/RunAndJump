...
{	
	...
	{		
		...
		protected virtual void AllCharactersAreDead()
		{
			...
			{
                // kiểm tra điểm có đạt được highscore không
                // kiểm tra game đang chơi là game gì
                // lấy highscore của game đang được lưu trong biến môi trường
                // so sánh highxore được lưu với điểm hiện tại. nếu ddieierm hiện tại > thì lưu thay thế điểm hiện tại vào highscore
                String gameName = SceneManager.GetActiveScene().name;
                if(PlayerPrefs.GetFloat(gameName) < _savedPoints)
                {
