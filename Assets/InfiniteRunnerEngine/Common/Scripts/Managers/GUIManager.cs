...
{	
	...
	public class GUIManager : Singleton<GUIManager>, MMEventListener<MMGameEvent>
	{
        ...
        /// the high score screen game object
		...
		{
            //PlayerPrefs.SetFloat("Run Shilin", 0);
            //PlayerPrefs.SetFloat("Run Buba", 0);
            //PlayerPrefs.SetFloat("Flappy Machito", 0);
            //PlayerPrefs.SetFloat("Fly Olek", 0);
            //PlayerPrefs.SetFloat("Jumping Hybrid", 0);
            RefreshPoints();
	        InitializeLives();
        ...
        /// </summary>
        /// <param name="state">If set to <c>true</c>, sets the game over screen on.</param>
        public virtual void SetHigeScoreScreen(bool state)
        {
            HighScoreScreen.SetActive(state);
            if(state) WinSound.Play();

             Text gameOverScreenTextObject = HighScoreScreen.transform.Find("GameOverScreenText").GetComponent<Text>();
            if (gameOverScreenTextObject != null)
            {
                gameOverScreenTextObject.text = "YOUR SCORE : " + Mathf.Round(GameManager.Instance.Points);
            }
        }
